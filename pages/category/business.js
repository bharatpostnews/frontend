...
const FETCH_ALL = gql`
  query postbyCategory($first: Int, $endCus: String, $catname1: String) {
    posts(first: $first, after: $endCus, where: { categoryName: $catname1 }) {
      pageInfo {
        endCursor
        hasNextPage
      }
...
function India({ LinksFetch, navLinks }) {
  const { data, loading, error, fetchMore } = useQuery(FETCH_ALL, {
    ...
    variables: { first: 3, endCus: null, catname1: "business" },
